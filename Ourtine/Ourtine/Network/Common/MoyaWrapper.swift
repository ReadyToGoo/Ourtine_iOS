            ...
            case .success(let response):
                ...
                if self.isDebugging {
                    ...
                    if let json = try? response.mapJSON() as? [String:Any] {
                        if let isSuccess = json["isSuccess"] as? Bool, let message = json["message"] as? String/*, let jsonResult = json["result"] */{
                ...
                if (200..<300).contains(response.statusCode) {
                    do {
                        ...
                        let decodeData = responseData.result
...
class TokenPlugin: PluginType {
    ...
    func prepare(_ request: URLRequest, target: TargetType) -> URLRequest {
        ...
        request.addValue("Authorization=Bearer%20\(token)", forHTTPHeaderField: "Cookie")
        return request
    }
}
